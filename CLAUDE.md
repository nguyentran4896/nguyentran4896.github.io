...
- **`components/`** contains one file per page section plus three cross-cutting pieces: `smooth-scroll.tsx` (Lenis-based smooth scroll wrapper), `custom-cursor.tsx` (bespoke pointer required on desktop, must respect `prefers-reduced-motion` and touch), `section-blend.tsx` (decorative transition between hero and About). `components/ui/` is the shadcn/Radix primitive library — many primitives are present but only a subset is actually used; do not assume importing one means it's wired up.
- **`components/sentient-sphere.tsx`** uses `@react-three/fiber` + `three` for the hero's animated 3D element. React Native / Expo packages appear in `package.json` but are not used by the web build — ignore them when working on the site.
